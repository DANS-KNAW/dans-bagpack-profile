...
2. (a) The bag MUST conform to the [DANS BagPack BagIt Profile]{:target=_blank} (even if `BagIt-Profile-Identifier` is missing). (b) The bag SHOULD
   conform to any other BagIt profiles declared in the `BagIt-Profile-Identifier` element.
